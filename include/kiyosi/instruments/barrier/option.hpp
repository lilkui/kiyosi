...
struct BarrierOptionTerms {
    ...
    double rebate{};
    kiyosi::rebate_timing rebate_timing{kiyosi::rebate_timing::at_expiry};
...
public:
    ...
    kiyosi::rebate_timing rebate_timing() const noexcept { return rebate_timing_; }
...
private:
    BarrierOption(option_type type, double strike, double rebate,
                  kiyosi::rebate_timing rebate_timing,
                  BarrierTerms barrier)
        : type_(type), strike_(strike), rebate_(rebate), rebate_timing_(rebate_timing),
    ...
    double strike_;
    double rebate_;
    kiyosi::rebate_timing rebate_timing_;
...
{
    auto barrier_terms = detail::make_barrier_terms(terms.barrier, terms.barrier_kind, terms.observation_mode,
    ...
    if (terms.rebate_timing != rebate_timing::at_hit && terms.rebate_timing != rebate_timing::at_expiry)
        return std::unexpected(Error{error_category::invalid_option, "invalid rebate timing"});
    if (barrier_terms->is_knock_in() && terms.rebate_timing == rebate_timing::at_hit)
        return std::unexpected(Error{error_category::invalid_option,
                                     "at-hit rebates are invalid for knock-in barriers"});
    return BarrierOption{terms.type, terms.strike, terms.rebate, terms.rebate_timing, std::move(*barrier_terms)};
}
