...
namespace {
using kiyosi::barrier_touch_status;
using kiyosi::test::day;

const auto effective = day(2025, 1, 1);
const auto expiry = day(2026, 1, 1); // Exactly one ACT/365 year.
                       ...
                       double expected, double tolerance = 1e-12)
{
    ...
    const kiyosi::FiniteDifferenceSettings grid{
        400, 400, kiyosi::finite_difference_scheme::crank_nicolson, 400.0};
    if constexpr (std::is_same_v<Instrument, kiyosi::Accumulator>) {
        ...
        {
            INFO("FD");
            check(kiyosi::FiniteDifferenceStructuredEngine<Instrument>{grid});
        }
        {
            INFO("MC");
            check(kiyosi::MonteCarloStructuredEngine<Instrument>{{64, 73}});
        }
    }
}

auto snowball(barrier_touch_status status)
{
    return kiyosi::make_snowball_option({.knock_out_coupon_rates = {0.20}, .maturity_coupon_rate = 0.12, .initial_price = 100.0, .knock_in_price = 80.0, .knock_out_prices = {120.0}, .upper_strike = 100.0, .lower_strike = 60.0, .observation_dates = {expiry}, .frequency = kiyosi::observation_frequency::daily, .touch_status = status, .effective = effective, .expiry = expiry});
}

auto ternary(barrier_touch_status status)
{
    return kiyosi::make_ternary_snowball_option({.knock_out_coupon_rates = {0.20}, .maturity_coupon_rate = 0.12, .minimal_coupon_rate = 0.03, .initial_price = 100.0, .knock_in_price = 80.0, .knock_out_prices = {120.0}, .upper_strike = 100.0, .lower_strike = 60.0, .observation_dates = {expiry}, .frequency = kiyosi::observation_frequency::daily, .touch_status = status, .effective = effective, .expiry = expiry});
}

auto binary(barrier_touch_status status)
{
    return kiyosi::make_binary_snowball_option({.knock_out_coupon_rates = {0.20}, .maturity_coupon_rate = 0.12, .initial_price = 100.0, .knock_out_prices = {120.0}, .upper_strike = 100.0, .lower_strike = 60.0, .observation_dates = {expiry}, .touch_status = status, .effective = effective, .expiry = expiry});
}

auto phoenix(barrier_touch_status status)
{
    return kiyosi::make_phoenix_option({.coupon_rate = 0.08, .initial_price = 100.0, .knock_in_price = 80.0, .knock_out_prices = {120.0}, .coupon_barriers = {90.0}, .upper_strike = 100.0, .lower_strike = 60.0, .observation_dates = {expiry}, .frequency = kiyosi::observation_frequency::daily, .touch_status = status, .effective = effective, .expiry = expiry});
}
} // namespace

TEST_CASE("FD-MC snowball families match independent expiry payoffs", "[cross-validation]")
{
    struct Case {
        double spot;
        double snowball;
    ...
    const auto standard = snowball(barrier_touch_status::none);
    const auto three_way = ternary(barrier_touch_status::none);
    const auto two_way = binary(barrier_touch_status::none);
    REQUIRE(standard);
    REQUIRE(three_way);
    REQUIRE(two_way);
    for (const auto& row : cases) {
        CAPTURE(row.spot);
        const auto context = market(row.spot, expiry);
        {
            INFO("snowball");
...
{
    const auto note = phoenix(barrier_touch_status::none);
    REQUIRE(note);
    ...
    for (const auto [spot, expected] : std::array{
        ...
        CAPTURE(spot);
        check_known_price(*note, market(spot, expiry), expected);
    }
}
...
TEST_CASE("FD-MC historical knock-in survives recovery and knock-out extinguishes notes",
          "[cross-validation]")
{
    const auto standard = snowball(barrier_touch_status::down);
    const auto three_way = ternary(barrier_touch_status::down);
    const auto bird = phoenix(barrier_touch_status::down);
    REQUIRE(standard);
    REQUIRE(three_way);
    REQUIRE(bird);
    ...
    for (const auto [spot, principal] : std::array{
        ...
        CAPTURE(spot);
        const auto context = market(spot, expiry);
    ...
    const auto check_extinguished = [](const auto& result) {
        REQUIRE(result);
        check_known_price(*result, market(100.0, effective), 0.0);
        check_known_price(*result, market(120.0, expiry), 0.0);
    };
    check_extinguished(snowball(barrier_touch_status::up));
    check_extinguished(ternary(barrier_touch_status::up));
    check_extinguished(binary(barrier_touch_status::up));
    check_extinguished(phoenix(barrier_touch_status::up));
}

TEST_CASE("FD-MC accumulator matches independent expiry quantity accounting", "[cross-validation]")
{
    const auto option = kiyosi::make_accumulator({100.0, 110.0, 1.0, 2.0, 3.0, effective, expiry});
    REQUIRE(option);
    ...
    for (const auto [spot, expected] : std::array{
        ...
        CAPTURE(spot);
        check_known_price(*option, market(spot, expiry), expected);
    }
}

TEST_CASE("FD-MC pre-expiry prices approach independently known constant-path limits",
          "[cross-validation]")
{
    ...
    const auto context = market(100.0, effective, 0.0, 1e-8);
    const auto check = [&](const auto& note, double expected) {
        REQUIRE(note);
        check_known_price(*note, context, expected, 1e-6);
    };
    check(snowball(barrier_touch_status::none), 1.12);
    check(ternary(barrier_touch_status::none), 1.12);
    check(binary(barrier_touch_status::none), 1.12);
    check(phoenix(barrier_touch_status::none), 9.0);

    const auto option = kiyosi::make_accumulator(
        {100.0, 110.0, 1.0, 2.0, 3.0, effective, day(2025, 1, 6)});
    REQUIRE(option);
    // Six trading observations (including valuation and expiry): (3 + 6*2)*(90-100).
    check_known_price(*option, market(90.0, effective, 0.0, 1e-8), -150.0, 1e-5);
}

TEST_CASE("FD-MC binary snowball discounts a known fixed terminal cashflow", "[cross-validation]")
{
    // The only observation is expiry and both outcomes pay 1.12, for every simulated path.
    const auto note = kiyosi::make_binary_snowball_option({.knock_out_coupon_rates = {0.12}, .maturity_coupon_rate = 0.12, .initial_price = 100.0, .knock_out_prices = {120.0}, .upper_strike = 100.0, .lower_strike = 60.0, .observation_dates = {expiry}, .effective = effective, .expiry = expiry});
    REQUIRE(note);
    check_known_price(*note, market(100.0, effective, 0.05), 1.12 * std::exp(-0.05), 1e-8);
}
...
TEST_CASE("FD-MC accumulator current knock-out settles immediately", "[cross-validation]")
{
    const auto option = kiyosi::make_accumulator(
        {100.0, 110.0, 1.0, 2.0, 3.0, effective, day(2025, 1, 6)});
    REQUIRE(option);
    check_known_price(*option, market(110.0, effective, 0.05), 30.0);
}
          ...
          "[cross-validation]")
{
    ...
    for (const double prior_quantity : {0.0, 3.0}) {
        ...
        const auto option = kiyosi::make_accumulator(
            {100.0, 110.0, 1.0, 2.0, prior_quantity, effective, day(2025, 1, 7)});
        REQUIRE(option);
        ...
        REQUIRE(fd_result);
        REQUIRE(mc_result);
        const auto fd_price = fd_result->require(kiyosi::risk_measure::price);
        const auto mc_price = mc_result->require(kiyosi::risk_measure::price);
        REQUIRE(fd_price);
        REQUIRE(mc_price);
...
TEST_CASE("FD-MC phoenix pays its terminal observation coupon exactly once", "[cross-validation]")
{
    ...
    const auto note = kiyosi::make_phoenix_option({.coupon_rate = 0.0025, .initial_price = 100.0, .knock_in_price = 80.0, .knock_out_prices = {120.0}, .coupon_barriers = {90.0}, .upper_strike = 100.0, .lower_strike = 60.0, .observation_dates = {maturity}, .frequency = kiyosi::observation_frequency::daily, .effective = start, .expiry = maturity});
    REQUIRE(note);
    ...
    const auto fd = kiyosi::FiniteDifferencePhoenixEngine{
        {400, 1600, kiyosi::finite_difference_scheme::crank_nicolson, 400.0}}
                        .price(*note, context);
