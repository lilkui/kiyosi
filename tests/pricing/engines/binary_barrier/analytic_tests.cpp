...
TEST_CASE("Touch factories require only payoff-relevant terms")
{
    ...
    const auto observation_dates = std::vector<kiyosi::date>{effective + std::chrono::days{30}, expiry};
    const auto cash = kiyosi::make_cash_one_touch_up(
        effective, expiry, 130.0, 10.0, kiyosi::settlement_timing::at_hit,
        kiyosi::observation_mode::scheduled, observation_dates);
    ...
    CHECK(cash->observation_mode() == kiyosi::observation_mode::scheduled);
