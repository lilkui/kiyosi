...
TEST_CASE("Finite-difference European engines track analytic prices")
{
    ...
    CHECK_THAT(risk_value(*european, kiyosi::RiskMeasure::price),
               WithinAbs(risk_value(analytic, kiyosi::RiskMeasure::price), 0.05));
    for (const auto scheme : {kiyosi::FiniteDifferenceScheme::explicit_euler,
        ...
        CHECK_THAT(risk_value(*result, kiyosi::RiskMeasure::price),
                   WithinAbs(risk_value(analytic, kiyosi::RiskMeasure::price), 0.15));
    }
}
...
TEST_CASE("Finite-difference American engines track analytic and binomial prices")
{
    ...
    CHECK_THAT(risk_value(*american, kiyosi::RiskMeasure::price),
               WithinAbs(risk_value(analytic, kiyosi::RiskMeasure::price), 0.05));
    ...
    CHECK_THAT(risk_value(*finite_put, kiyosi::RiskMeasure::price),
               WithinAbs(risk_value(*tree_put, kiyosi::RiskMeasure::price), 0.1));
}
...
TEST_CASE("Finite-difference American engine returns intrinsic value at expiry_date")
{
    ...
    CHECK(risk_value(*at_expiry, kiyosi::RiskMeasure::price) == 10.0);
}
