...
STABILITY = dict(price=0.001, delta=0.0002, gamma=0.00005, speed=0.00002,
                 theta=0.00003, charm=0.00002, color=0.000002, vega=0.001,
                 vanna=0.00003, zomma=0.00001, rho=0.001)
ENGINES = {
    "BinomialAmericanEngine": {"steps"}, "CrrEngine": {"steps"},
    "FiniteDifferenceAmericanEngine": {"asset_steps", "time_steps", "scheme", "upper_boundary"},
    "BjerksundStenslandAmericanEngine": set(),
    "MonteCarloAmericanEngine": {"seed", "paths", "steps"},
}
LEGACY = {"american-binomial": dict(steps=200),
          "american-fd": dict(asset_steps=200, time_steps=4000, scheme="explicit_euler", upper_boundary=0),
          "american-bs": {}, "american-mc": dict(seed=42, paths=20000, steps=50)}
...
def native(encoded, grid):
    ...
    g.require(all(math.isfinite(v) for v in result.values()), "non-finite American result")
    return result
...
def measure(inputs, name, grid=GRIDS[-1], scale=1):
    ...
    return (measure(g.shifted(inputs, field, h), base, grid) -
            measure(g.shifted(inputs, field, -h), base, grid)) / (2 * h * unit)
    ...
    for name in g.MEASURES:
        ...
        if name in {"delta", "gamma"}:
            ...
            for h in (0.5, 1.0):
                ...
                estimates.append((up - down) / (2 * h) if name == "delta" else
                                 (up - 2 * measure(inputs, "price") + down) / h**2)
        else:
        ...
        g.require(all(math.isfinite(v) for v in [*values, *estimates]), f"non-finite American {name}")
        g.require(uncertainty <= STABILITY[name], f"unstable American {name}: {uncertainty}, inputs={inputs}")
        outputs[name] = value
        metadata.update({f"uncertainty_{name}": uncertainty, f"refinement_{name}": refinement,
                         f"coarse_refinement_{name}": abs(values[1] - values[0]),
                         f"bump_error_{name}": bump_error, f"stability_limit_{name}": STABILITY[name]})
    return outputs, metadata


def provenance(inputs):
    return dict(inputs, source_revision=f"QuantLib-{ql.__version__}",
                source_symbol="QuantLib.FdBlackScholesVanillaEngine", convention=g.CONVENTION,
                reference_kind="discretized", reference_classification="convergence-verified-contract",
...
def rows():
    ...
    g.require({p["engine"] for p in data["profiles"]} == set(ENGINES) and
              len(data["profiles"]) == len(ENGINES), "incomplete American engines")
    seen = set()
    for scenario in data["scenarios"]:
        g.require(set(scenario) == {"case_id", "inputs", "wrapper"}, "invalid American scenario")
        g.require(scenario["case_id"].startswith("ql-american-") and scenario["case_id"] not in seen,
        ...
        g.validate_scenario(dict(case_id=scenario["case_id"].replace("ql-american-", "ql-european-"),
                                inputs=inputs, tolerances=STABILITY, numerical_tolerances=STABILITY))
        outputs, metadata = reference(inputs)
        for profile in data["profiles"]:
            engine = profile["engine"]
            g.require(set(profile) == {"engine", "settings", "shifts", "tolerances", "numerical_tolerances"},
                      "invalid American profile")
            g.require(set(profile["settings"]) == ENGINES[engine], "invalid American settings")
            for key, value in profile["settings"].items():
                g.require(value == "crank_nicolson" if key == "scheme" else
                          type(value) is int and value > 0, "invalid American setting")
            g.require(set(profile["shifts"]) == {"spot_shift", "volatility_shift", "rate_shift", "time_shift_days"},
                      "invalid American shifts")
            g.require(all(type(v) in (int, float) and math.isfinite(v) and v > 0 for v in profile["shifts"].values())
                      and type(profile["shifts"]["time_shift_days"]) is int, "invalid American shift")
            for field in ("tolerances", "numerical_tolerances"):
                g.require(set(profile[field]) == set(g.MEASURES) and
                          all(type(v) in (int, float) and math.isfinite(v) and v >= 0
                              for v in profile[field].values()), "invalid American budgets")
            attributes = provenance(inputs)
            attributes.update(metadata)
            attributes.update(profile["settings"])
            attributes.update(profile["shifts"])
            attributes.update(owner="QuantLib", wrapper=str(scenario["wrapper"]).lower(),
            ...
            attributes.update({f"numerical_tolerance_{name}": profile["numerical_tolerances"][name]
                               for name in outputs})
            mc = "|".join(str(attributes[key]) for key in ("seed", "paths", "steps", "tolerance")) if engine == "MonteCarloAmericanEngine" else "-"
            yield "\t".join([scenario["case_id"] + "-" + engine.lower(), "AmericanOption", engine,
                              inputs["option"], g.encode(attributes), g.encode(outputs),
                              g.encode({name: profile["tolerances"][name] for name in outputs}), "-", "-", mc])
...
def migrate(line):
    ...
    inputs = dict(option=old["option"], strike=100, spot=100, rate=0.04, dividend=0.01,
                  volatility=0.3, effective="2024-12-30", valuation="2025-01-06", expiry="2026-01-06")
    for key in g.INPUTS & old.keys():
        inputs[key] = float(old[key]) if key in {"strike", "spot", "rate", "dividend", "volatility"} else old[key]
    ...
    old.update(reference_provider="QuantLib", reference_uncertainty=uncertainty,
               tolerance_rationale="retained absolute comparison budget: GENERATION.md")
    fields[4] = g.encode(old)
    fields[5] = g.encode(dict(price=prices[-1]))
    if fields[8] != "-":
