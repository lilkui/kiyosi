        ...
        if field == "valuation":
            if "averaging" in inputs:
                require(date.fromisoformat(inputs["valuation"]) - timedelta(days=h) >
                        date.fromisoformat(inputs["average_start"]), "time stencil touches averaging start")
            require(date.fromisoformat(inputs["valuation"]) + timedelta(days=h) <
...
def european_row(scenario, stability=STABILITY):
    ...
    import binary
    import asian
    unavailable = asian.exclusions(inputs) if "averaging" in inputs else binary.exclusions(inputs) if "asset_settlement" in inputs else dict.fromkeys(
...
def validate_manifest(text):
    ...
    for line in lines[1:]:
        ...
        if owned:
            ...
            require(is_asian or is_binary or is_barrier or is_american or is_digital or fields[1] == "EuropeanOption", "unknown generated instrument")
            limits = asian.STABILITY if is_asian else binary.STABILITY if is_binary else barrier.STABILITY if is_barrier else american.STABILITY if is_american else digital.STABILITY if is_digital else STABILITY
            days = (date.fromisoformat(inputs["expiry"]) - date.fromisoformat(inputs["valuation"])).days
            unavailable = asian.exclusions(inputs) if is_asian else binary.exclusions(inputs) if is_binary else american.exclusions(inputs) if is_american else dict.fromkeys(
...
def regenerate(fixture=FIXTURE, scenarios_path=PROJECT / "scenarios.json",
    ...
    retained = [asian.migrate(binary.migrate(barrier.migrate(digital.migrate(american.migrate(migrate_numerical(line)))))) for line in original.splitlines() if not line.startswith("ql-")]
