...
def exclusions(inputs):
    ...
    return dict.fromkeys(g.TIME_MEASURES if days <= 2 else (), "whole-day stability stencil touches expiry")


def option(inputs):
    contract = make_contract(inputs)
    return SimpleNamespace(NPV=lambda: price(tuple(sorted(inputs.items()))),
        **{name: getattr(contract, name) for name in ("delta", "gamma", "theta", "vega", "rho") if hasattr(contract, name)})
...
def make_contract(inputs):
    ...
    g.require(kind in barrier.KINDS and direction in {"call", "put", "none"}, "unknown binary contract")
    g.require(inputs["monitoring"] == "continuous", "scheduled binary monitoring has no matching reference")
    g.require(inputs["asset_settlement"] in {"true", "false"}, "unknown binary payoff")
    g.require(inputs["settlement"] in {"at_hit", "at_expiry"}, "unknown binary settlement")
    asset = inputs["asset_settlement"] == "true"
    knock_in, up = kind.endswith("_in"), kind.startswith("up")
    at_hit = inputs["settlement"] == "at_hit"
    g.require(not at_hit or knock_in and direction == "none", "hit payment requires a one-touch")
    g.require(not asset or not at_hit or inputs["payout"] == inputs["barrier"], "asset hit payment must equal barrier")
    valuation, expiry = (ql.DateParser.parseISO(inputs[key]) for key in ("valuation", "expiry"))
    ql.Settings.instance().evaluationDate = valuation
    hit = inputs["spot"] >= inputs["barrier"] if up else inputs["spot"] <= inputs["barrier"]
    def cash(amount):
    ...
    if valuation == expiry:
        ...
        payoff = (ql.AssetOrNothingPayoff(side, inputs["strike"]) if asset else
                  ql.CashOrNothingPayoff(side, inputs["strike"], inputs["payout"]))
    ...
    process = ql.BlackScholesMertonProcess(ql.QuoteHandle(ql.SimpleQuote(inputs["spot"])),
        curve(inputs["dividend"]), curve(inputs["rate"]), ql.BlackVolTermStructureHandle(
    ...
    if at_hit:
        ...
        payoff = ql.CashOrNothingPayoff(ql.Option.Call if up else ql.Option.Put,
                                     inputs["barrier"], inputs["payout"])
        contract = ql.VanillaOption(payoff, ql.AmericanExercise(valuation, expiry, False))
    ...
    def value(side):
        ...
        if hit:
            ...
            contract = ql.BarrierOption(barrier.KINDS[kind], inputs["barrier"], 0, payoff,
    ...
    return SimpleNamespace(**{name: lambda name=name: sum(getattr(component, name)() for component in components)
    ...
    for asset in (False, True):
        for kind in barrier.KINDS:
            for direction in ("none", "call", "put"):
                settlements = ("at_expiry", "at_hit") if direction == "none" and kind.endswith("_in") else ("at_expiry",)
                for settlement in settlements:
                    ...
                    points = [(80, 100, 90), (100, 100, 365), (120, 100, 730),
                              (100, 150 if kind.startswith("up") else 50, 365),
                              (level, 100, 365), (level + (10 if kind.startswith("up") else -10), 100, 365),
                              (110, 100, 1)]
                    if settlement == "at_expiry":
                        points += [(100, 100, 0), (100, 95, 0), (level, 100, 0)]
                    for spot, strike, days in points:
                        inputs = dict(option=direction, strike=strike, spot=spot, rate=0.04, dividend=0.01,
                            volatility=0.3, effective="2024-12-30", valuation="2025-01-06",
                            expiry=(date(2025, 1, 6) + timedelta(days=days)).isoformat(),
                            barrier_kind=kind, barrier=100 if days == 0 and spot == strike else level,
                            payout=level if asset and settlement == "at_hit" else 10,
                            asset_settlement=str(asset).lower(), settlement=settlement, monitoring="continuous")
                        yield f"ql-binary-{'asset' if asset else 'cash'}-{direction}-{kind.replace('_', '-')}-{settlement.replace('_', '-')}-{spot}-{strike}-{days}d", inputs
...
def rows():
    for identifier, inputs in scenarios():
        fields = g.european_row(dict(case_id=identifier, inputs=inputs, tolerances=BUDGET,
                                    numerical_tolerances=BUDGET), STABILITY).split("\t")
        fields[1:3] = ["BinaryBarrierOption", "AnalyticBinaryBarrierEngine"]
        terms = g.attributes(fields[4])
        terms.update(metadata(), spot_shift=0.02, volatility_shift=0.0001, rate_shift=0.0001, time_shift_days=1,
...
def migrate(line):
    fields = line.split("\t")
    if len(fields) != 10 or fields[0].startswith("ql-") or fields[1] != "BinaryBarrierOption" or fields[5] == "-":
        return line
    terms = g.attributes(fields[4])
    if terms.get("monitoring") == "scheduled":
        return line
    g.require(fields[2] == "AnalyticBinaryBarrierEngine", "unknown binary engine")
    inputs = dict(option=terms.get("option", "none"), strike=float(terms["strike"]), spot=100, rate=0.04,
        dividend=0.01, volatility=0.3, effective="2024-12-30", valuation="2025-01-06", expiry=terms["expiry"],
        barrier=float(terms["barrier"]), barrier_kind=terms.get("barrier_kind", "down_and_in"), payout=float(terms["payout"]),
        asset_settlement=terms.get("asset_settlement", "false"), settlement=terms.get("settlement", "at_expiry"), monitoring="continuous")
...
def check_bindings():
    ...
    ql.Settings.instance().evaluationDate = valuation
    curve = lambda rate: ql.YieldTermStructureHandle(ql.FlatForward(valuation, rate, ql.Actual365Fixed()))
    process = ql.BlackScholesMertonProcess(ql.QuoteHandle(ql.SimpleQuote(100)), curve(.01), curve(.04),
        ql.BlackVolTermStructureHandle(ql.BlackConstantVol(valuation, ql.NullCalendar(), .3, ql.Actual365Fixed())))
    for engine in (ql.AnalyticBinaryBarrierEngine, ql.AnalyticDigitalAmericanEngine):
        for payoff in (ql.CashOrNothingPayoff(ql.Option.Call, 130, 10), ql.AssetOrNothingPayoff(ql.Option.Call, 130)):
            for exercise, error in ((ql.EuropeanExercise(expiry), "non-American exercise given"),
                    (ql.AmericanExercise(valuation, expiry, False), "payoff must be at expiry" if engine == ql.AnalyticBinaryBarrierEngine else None),
                    (ql.AmericanExercise(valuation, expiry, True), None),
                    (ql.AmericanExercise(valuation + 1, expiry, True), "American option with window exercise not handled yet")):
                contract = (ql.BarrierOption(ql.Barrier.UpIn, 130, 0, payoff, exercise)
                            if engine == ql.AnalyticBinaryBarrierEngine else ql.VanillaOption(payoff, exercise))
    ...
    for side, level, kind in ((ql.Option.Call, 130, "up_and_in"), (ql.Option.Put, 70, "down_and_in")):
        for asset in (False, True):
            for deferred in (False, True):
                payout = level if asset else 10
                payoff = ql.AssetOrNothingPayoff(side, level) if asset else ql.CashOrNothingPayoff(side, level, payout)
                contract = ql.VanillaOption(payoff, ql.AmericanExercise(valuation, expiry, deferred))
                contract.setPricingEngine(ql.AnalyticDigitalAmericanEngine(process))
                inputs = dict(option="none", strike=100, spot=100, rate=.04, dividend=.01, volatility=.3,
                    valuation="2025-01-06", expiry="2026-01-06", effective="2024-12-30", barrier=level,
                    barrier_kind=kind, payout=payout, asset_settlement=str(asset).lower(), monitoring="continuous",
                ...
                if not deferred:
                    for name, unit in (("delta", 1), ("gamma", 1), ("rho", 100)):
                        ...
                        assert abs(g.measure(inputs, name, price_only=True) - direct) < STABILITY[name]
                else:
                    ...
                    for name, unit in (("delta", 1), ("gamma", 1), ("theta", 365), ("vega", 100), ("rho", 100)):
                        direct = sum(getattr(g.european_option(dict(touched, option=direction,
                            payoff="asset" if asset else "cash")), name)() for direction in ("call", "put")) / unit
                        assert abs(g.measure(touched, name) - direct) < 1e-12
                        assert abs(g.measure(touched, name, price_only=True) - direct) < STABILITY[name]
    # An unsupported schedule must retain its original expected value and provenance.
    legacy = next(line for line in g.FIXTURE.read_text().splitlines() if line.startswith("binary-barrier\t"))
    fields = legacy.split("\t")
    terms = g.attributes(fields[4])
